...
var link = "img/mage-sprite-8.png";//download
...
questionmark.add_event_listener( "click" , function() {
    toast.makeToast("Tente derrotar todos os inimigos!");
});
...
var update_stats = function() {
    ...
    if(health < 0) {
        ...
        update_stats();
        toast.makeToast("Fim de Jogo", 1000);
        setTimeout(function() {
            load_level(level);
        },50);
    }
    ...
    var setup = function() {
        ...
        var change_func = function(e) {
            if(is_targeted(e.target, change_div)) {
                ...
                if(color === "rgb(0, 0, 0)") {
                    pause();
                    toast.makeToast("Arraste os botões de controle para reposicioná-los");
...
var load_level = function(id) {
    ...
    if(lvl && (coin_counter >= lvl.cost || lvl.unlocked)) {
        ...
        toast.makeToast("Você precisa "+lvl.cost
        +" moedas para entrar no nível "+id+"!");
    } else {
        toast.makeToast("Level "+id+" ainda não existe");
    }
};
